...
if __name__ == "__main__":
    dataset = np.load('dataset_v3.npy', allow_pickle=True)[0]
    ...
    simforest = SimilarityForest(similarity_function=cos_sim)
    simforest.fit(traindata.get('data_input'), traindata.get('data_output'))

    y = simforest.predict(X)
    error = Y - y
    accuracy = 1- len(np.where(error != 0)[0]) /len(X) 
